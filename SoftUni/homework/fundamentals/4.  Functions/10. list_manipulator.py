...
def exchange_list_at_index(collection: list, index: int):
    ...
    if not is_valid(collection, index):
        print("Invalid index")
...
while not command == "end":
    ...
    if cmd_type == "exchange":
        ...
        cmd_count = int(cmd_args[1])
        cmd_filter = cmd_args[2]

        if cmd_filter == "even":
            first_count_elements_by_criteria(numbers, cmd_count, lambda n: n % 2 == 0)
        elif cmd_filter == "odd":
            first_count_elements_by_criteria(numbers, cmd_count, lambda n: n % 2 != 0)
        ...
        if cmd_filter == "even":
            last_count_elements_by_criteria(numbers, cmd_count, lambda n: n % 2 == 0)
